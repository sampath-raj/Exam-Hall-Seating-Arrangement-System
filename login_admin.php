<?php
session_start();
require_once('db.php'); // Make sure this is at the top of the file

if(isset($_POST['submit'])) {
    // Ensure $conn exists before using mysqli_real_escape_string
    if($conn) {
...
<head>
    ...
    <style>
        ...
        .submit {
            ...
            border-radius: 5em;
            color: #fff;
            background: linear-gradient(to right, #0D47A1, #2196F3);  /* Dark blue to light blue gradient */
            border: 0;
            padding: 10px 40px;
            margin-top: 10px;
            margin-left: 35%;
